		...
		for (i=0; i< flushDraws.length; i++) {
			if (aCards.length === 5) { //flop
				if (flushDraws[i].length === 4) {
					returnHands.push({rank: nsDrawingHand.FLUSH_DRAW, high: flushDraws[i][0].rank, low: flushDraws[i][flushDraws[i].length-1].rank});
					continue;
				}
				if (flushDraws[i].length === 3) {
					if(!nsDrawingHand.bIgnoreBackdoorDraws)
						returnHands.push({rank: nsDrawingHand.BACKDOOR_FLUSH_DRAW, high: flushDraws[i][0].rank, low: flushDraws[i][flushDraws[i].length-1].rank});
					continue;
				}
...
nsDrawingHand.fFindBestStraightDraw = function(aCards) {
	...
	for(var i=0; i< aFoundDrawingStraights.length; i++) {
		...
		aFoundDraws.push({rank: rank, high: iHigh, low: iLow});		
	}	
