...
var Pair = AWModel.extend({
	...
	toArray: function(aFilter) {
		...
		if(rank1 === rank2) {
			aCards.push( {rank:rank2,suit:1} );
			aCards.push( {rank:rank2,suit:2} );
			aCards.push( {rank:rank2,suit:3} );
			aCards.push( {rank:rank2,suit:4} );
			...
			aCards.push( {rank:rank2,suit:3} );
			aCards.push( {rank:rank2,suit:4} );
			aCards.push( {rank:rank1,suit:1} );
			aCards.push( {rank:rank1,suit:2} );
			aCards.push( {rank:rank1,suit:3} );
			aCards.push( {rank:rank1,suit:4} );
			aAllKinds = fCombinatorics(aCards,2);	
		}
