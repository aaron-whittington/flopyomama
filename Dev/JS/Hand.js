...
oHand = {
   rank:0,
   high:-1, /*for all hands high card in the made portion of the hand*/
   low:-1,  /*for two pair and fh, the low card in the made portion*/
   kickers: [], /*up to five cards for high-card hands*/
   subtype: -1, /*types such as over pair*/
   drawinghands: []
};
...
nsHand.fGetBestHand = function(aCards) {
	var oNsHand = nsHand;
	if (aCards.length ===1) {
		return {rank: oHand.HIGH_CARD, high: aCards[0].rank, low:-1, kickers:[]};			
	}
...
nsHand.fFindBestSet = function(aCards) {
    ...
    for (i= 14; i>= 2; i--) {
        ...
        if(timesFound >= 2)
			aSets.push({same: foundRank, kickers: notFoundRank});
    }
...
nsHand.fFindHighCards = function(aCards) {
    var highRank = [], cardsLength = aCards.length;
    for (i=0; i<cardsLength; i++)
        highRank[i] = -1;
    for (i=0; i<cardsLength; i++) {
        var foundIndex = -1, newCardsLength = aCards.length;
