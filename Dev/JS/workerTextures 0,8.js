  ...
  switch (data.cmd) {
    case 'start':
	  ...
	  if(data.bMin) 
		...
		importScripts('Maths.js', 'Hand.js','DrawingHand.js','Convert.js');
...
nsWorkerTextures.fCalculateBoards = function (aoStartingHands,aKnownCards,aFixedBoardCards, oFilter) {		
	...
	var oPairLengthDic = {};
		...
		for (var iVillainPair = startingHandLengths-1; iVillainPair >= 0; iVillainPair--) {
			...
			for(var i=0;i<oPairArray.length;i++){				
						...
						var oVillainHand = nsDrawingHand.fGetDrawingHands(aVillainHand);
	...
	self.fPostDone({oVillainStat: oVillainStaticDic, count: numberDone, oPairLengthDic : oPairLengthDic});
}
